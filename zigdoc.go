...
func writeZigDocs(root, version, channel string, stdout io.Writer) {
	...
	if err := writeFileAtomic(promptPath, []byte(adaptationPrompt(version, versionDir)), 0o644); err != nil {
...
func adaptationPrompt(version, versionDir string) string {
	var b strings.Builder
	...
	b.WriteString("---\n\n")
	fmt.Fprintf(&b, "You are adapting to Zig %s. Read these local files first:\n\n", version)
